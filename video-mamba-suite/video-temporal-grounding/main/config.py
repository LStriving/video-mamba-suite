...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--device", type=str, default='cuda', help="0 cuda, -1 cpu")
    ...
    def display_save(self, opt):
        ...
        if not os.path.exists(opt.results_dir):
            ...
            print(f"fold '{opt.results_dir}' made")
    ...
    def parse(self, args=None):
        ...
        if opt.save_dir != "":
            opt.results_dir = os.path.join(opt.results_root, opt.save_dir + '_' + str(opt.depth)) # debug mode.
...
class TestOptions(BaseOptions):
    ...
    def initialize(self):
        ...
        self.parser.add_argument("--eval_results_dir", type=str, default='./output_pretrain',
...
class WarmupStepLR(torch.optim.lr_scheduler.StepLR):
    def __init__(self, optimizer, warmup_steps, step_size, gamma=0.2, last_epoch=-1):
...
def make_optimizer(model, opt):
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            elif pn.endswith('A_log'):
                # all biases will not be decayed
                no_decay.add(fpn)
            elif pn.endswith('D'):
                no_decay.add(fpn)
                # all biases will not be decayed
            elif pn.endswith('A_b_log'):
                # all biases will not be decayed
                no_decay.add(fpn)
            elif pn.endswith('D_b'):
                # all biases will not be decayed
                no_decay.add(fpn)
            else:
                decay.add(fpn)
    ...
    optim_groups = [
        {"params": [param_dict[pn] for pn in sorted(list(decay))], "weight_decay": opt.wd},
        {"params": [param_dict[pn] for pn in sorted(list(no_decay))], "weight_decay": 0.0},
    ]

    optimizer = torch.optim.AdamW(optim_groups, lr=opt.lr)
...
def setup_model(opt):
    ...
    optimizer = make_optimizer(model,opt)
    #soptimizer = torch.optim.AdamW(param_dicts, lr=opt.lr,weight_decay=opt.wd)
