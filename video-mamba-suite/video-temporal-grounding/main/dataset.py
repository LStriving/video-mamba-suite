...
class DatasetVLP(Dataset):
    ...
    def __init__(self, dset_name, data_path, v_feat_dirs, q_feat_dir, v_feat_dim, q_feat_dim,
                 ...
                 use_cache=-1, fix_len=-1, add_easy_negative=1, easy_negative_only=-1,feat_root='root'):
        ...
        self.vlp_mapping = {
            ...
            '/yourrootpath/grounding_meta/point_egoclip_wo_val.jsonl': {      
                'dset_name': 'ego4d', 'v_feat_suffix': '_point', 'q_feat_suffix': '_point', 'type': 'point',
            },
            '/yourrootpath/grounding_meta/interval_900k.jsonl': {      
                'dset_name': 'videocc', 'v_feat_suffix': '', 'q_feat_suffix': '', 'type': 'interval',
            },
            '/yourrootpath/grounding_meta/curve_5_window.jsonl': {      
                'dset_name': 'videocc', 'v_feat_suffix': '', 'q_feat_suffix': '_concept', 'type': 'curve',
            },
            # downstream
            '/yourrootpath/grounding_meta/qvhighlights/qvhighlights_val.jsonl': {      
                'dset_name': 'QVHighlights', 'v_feat_suffix': '', 'q_feat_suffix': '', 'type': 'curve',
            },
            '/yourrootpath/grounding_meta/qvhighlights/qvhighlights_train.jsonl': {      
                'dset_name': 'QVHighlights', 'v_feat_suffix': '', 'q_feat_suffix': '', 'type': 'curve',
            },
            '/yourrootpath/grounding_meta/Charades-STA/charades_train.jsonl': {      
                # 'dset_name': 'charades', 'v_feat_suffix': '_2', 'q_feat_suffix': '', 'type': 'interval',
                'dset_name': 'Charades-STA', 'v_feat_suffix': '', 'q_feat_suffix': '', 'type': 'interval',
            },
            '/yourrootpath/grounding_meta/NLQ/nlq_train.jsonl': {
                'dset_name': 'ego4d', 'v_feat_suffix': '', 'q_feat_suffix': '', 'type': 'interval',
            },
            '/yourrootpath/grounding_meta/TACoS/train.jsonl': {      
            ...
            '/yourrootpath/grounding_meta/DiDeMo/train.jsonl': {      
    ...
    def _get_query_feat_by_qid(self, meta):
        ...
        q_feat_path = os.path.join(self.s3_path, dset_name, q_feat_dir, f"{qid}.npz")
        try: 
            q_feat = mmengine.get(q_feat_path)
            q_feat = io.BytesIO(q_feat)
            q_feat = np.load(q_feat)[self.q_feat_type].astype(np.float32)
        except:
            q_feat = np.zeros((10, self.q_feat_dim)).astype(np.float32)
    ...
    def _get_video_feat_by_vid(self, meta):
        ...
        for feat_type, _feat_dir in zip(self.v_feat_types, self.v_feat_dirs):
            ...
            if self.use_cache > 0:
                ...
                _feat_path = os.path.join(self.s3_path, dset_name, v_feat_dir, f"{vid}.npz")

                _feat = mmengine.get(_feat_path)
                _feat = io.BytesIO(_feat)
                _feat = np.load(_feat)["features"].astype(np.float32)
                if self.normalize_v:
...
class DatasetMR(Dataset):
    ...
    def _get_query_feat_by_qid(self, qid):
        ...
        q_feat_path = os.path.join(self.s3_path, self.q_feat_dir, f"{qid}.npz")
    ...
    def _get_video_feat_by_vid(self, vid):
        ...
        for feat_type, _feat_dir in zip(self.v_feat_types, self.v_feat_dirs):
            if self.use_cache > 0:
                ...
                _feat_path = os.path.join(self.s3_path, _feat_dir, f"{vid}.npz")
                _feat = mmengine.get(_feat_path)
                _feat = io.BytesIO(_feat)
                _feat = np.load(_feat)["features"].astype(np.float32)
                if self.normalize_v:
    ...
    def _get_video_feat_by_vid(self, vid):
        ...
        for feat_type, _feat_dir in zip(self.v_feat_types, self.v_feat_dirs):
            if self.use_cache > 0:
                ...
                _feat_path = os.path.join(self.s3_path, self.dset_name, _feat_dir, f"{vid}.npz")
                _feat = mmengine.get(_feat_path)
                _feat = io.BytesIO(_feat)
                _feat = np.load(_feat)["features"].astype(np.float32)
    ...
    def _get_query_feat_by_qid(self, qid):
        if self.use_cache > 0:
            try:
                ...
                q_feat = np.zeros((10, self.q_feat_dim)).astype(np.float32)
            return  torch.from_numpy(q_feat)
        q_feat_path = os.path.join(self.s3_path, self.dset_name, self.q_feat_dir, f"{qid}.npz")
