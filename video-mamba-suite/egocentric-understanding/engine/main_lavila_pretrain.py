...
def get_args_parser():
    ...
    parser.add_argument('--dataset-val', default='ek100_mir', type=str, choices=['ek100_mir', 'egoschema'])
    ...
    parser.add_argument('--pretrain-zoo', default='openai', type=str, choices=['openai', 'open_clip', 'lavila', 'deit'])
    ...
    parser.add_argument('--evaluate-task', default='ek100_mir', type=str, help='task to evaluate',choices=["ek100_mir", "egoschema"])    
...
def main(args):
        ...
        if os.path.isfile(args.resume):
            ...
            if checkpoint['args'].clip_length != args.clip_length:
                if "TimeSformer" in args.model:
                    # temporal embedding needs to be resized
                    print("Resizing temporal embedding for TimeSformer-style model")
                    load_temporal_embedding = checkpoint['state_dict']['module.visual.temporal_embed']
                    load_temporal_embedding = load_temporal_embedding.permute(0, 2, 1)
                    new_temporal_embed = F.interpolate(load_temporal_embedding, size=(args.clip_length,), mode='linear').permute(0, 2, 1)
                    checkpoint['state_dict']['module.visual.temporal_embed'] = new_temporal_embed
                elif "VIT" in args.model:
                    print("Resizing temporal embedding for Joint-ST ViT model")
                ...
                elif "ViMamba" in args.model or "ViViM" in args.model:
                    print("Resizing temporal embedding for ViViM model")
                    load_temporal_embedding = checkpoint['state_dict']['module.visual.temporal_embedding']
                    load_temporal_embedding = load_temporal_embedding.permute(1, 2, 0)
                    new_temporal_embed = F.interpolate(load_temporal_embedding, size=(args.clip_length,), mode='linear').permute(2, 0, 1)
                    checkpoint['state_dict']['module.visual.temporal_embedding'] = new_temporal_embed
                else:
                    print("No temporal embedding")
                print(f"{checkpoint['args'].clip_length} => {args.clip_length}")
    ...
    val_dataset = VideoCaptionDatasetCLIP(
        args.dataset_val, args.root_val, args.val_metadata,
    ...
    if args.evaluate:
        val_stats = validate_selectd_task(val_loader, val_transform_gpu, model, criterion, args)
...
def validate_egoschema(val_loader, transform_gpu, model, args):
    all_probs = []
    all_quids = []
    with amp.autocast(enabled=not args.disable_amp, dtype=torch.bfloat16 if args.use_bf16 else torch.float16):
        with torch.no_grad():
            for i, inputs in enumerate(val_loader):
                ...
                uids = inputs[0]
                videos = inputs[1].cuda(non_blocking=True)
                texts = inputs[2].cuda(non_blocking=True).long()
                ...
                num_answer = texts.shape[1]

                texts = einops.rearrange(texts,"b k d -> (b k) d")
                
                    
                image_features, text_features, logit_scale = model(videos, texts)
                video_feat = image_features / image_features.norm(dim=-1, keepdim=True)
                text_feat = text_features / text_features.norm(dim=-1, keepdim=True)
                
                text_feat = einops.rearrange(text_feat, '(b k) d -> b k d', b=batch_size)
                # [b, d] -> [b, k, d]
                # print(video_feat.shape)
                # print(text_feat.shape)
                logits = video_feat @ text_feat.transpose(-1, -2)
                preds = logits.max(2)[-1]
                target_indices = np.arange(batch_size)
                predictions = preds[target_indices, target_indices].cpu().numpy()
                # print(predictions, uids)
                
                all_probs.extend(predictions)
                all_quids.extend(uids)
                if i % args.print_freq == 0:
                    print(i)
                # break
    ...
    if dist_utils.get_world_size() > 1:
        res = list(zip(all_quids, all_probs))
        res = dist_utils.collect_results_cpu(res, len(val_loader.dataset))
        all_quids, all_probs = zip(*res)

    
    # save csv for kaggle
    csv = pd.DataFrame({'q_uid': all_quids, 'answer': all_probs})
    savename = os.path.join(args.output_dir, f'egoschema_results_{args.model}_f{args.clip_length}.csv')
    csv.to_csv(savename, index=False)
    
    # save json for valiation
    result_dict = {}
    for (uid, pred) in zip(all_quids, all_probs):
        result_dict[uid] = int(pred)
    savename = os.path.join(args.output_dir, f'egoschema_results_{args.model}_f{args.clip_length}.json')
    json.dump(result_dict, open(savename, 'w'))
    print('Done saving')
    from avion.evaluation.validate_egoschema import send_post_request
    resp = send_post_request(savename)
    print(f"Response Status Code: {resp.status_code}")
    print(f"Response Content:\n{resp.text}")
    

def validate_selectd_task(val_loader, transform_gpu, model, criterion, args):
    if args.evaluate_task == 'ek100_mir':
        return validate_mir(val_loader, transform_gpu, model, criterion, args)
    elif args.evaluate_task == 'egoschema':
        return validate_egoschema(val_loader, transform_gpu, model, args)
    else:
        raise ValueError(f'Unknown task {args.evaluate_task}')
