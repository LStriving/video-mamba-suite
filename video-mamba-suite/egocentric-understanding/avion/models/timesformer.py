...
class SpaceTimeBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if not use_flash_attn:
            ...
            self.timeattn = nn.MultiheadAttention(dim, num_heads, dropout=attn_drop)
        else:
            ...
            self.timeattn = FlashMHA(dim, num_heads, cross_attn=False, qkv_proj_bias=True,
                                 out_proj_bias=True, dropout=attn_drop, use_flash_attn=True)
    ...
    def forward(self, x, time_n, space_f, use_checkpoint=False):
        ...
        if not self.use_flash_attn:
            x_ = self.norm3(x)
            time_output = self.timeattn(x_, x_, x_, need_weights=False)[0]
        else:
        ...
        if self.attention_style == 'frozen-in-time':
            x = res_x + self.drop_path(torch.cat((cls_token, space_output), 1))
        elif self.attention_style == 'timesformer-div':
            # todo: add drop path for timesformer-dev style
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        assert attention_style in ['frozen-in-time', 'timesformer-div']
        ...
        self.blocks = nn.ModuleList([
            SpaceTimeBlock(
                ...
                attention_style=attention_style, act_layer=act_layer, is_tanh_gating=is_tanh_gating, use_flash_attn=use_flash_attn)
    ...
    def forward_features(self, x, cls_at_last=True, return_feat_3d=False):
        B, curr_frames, channels, _, _ = x.shape
        # print(x.shape)
        x, T, W = self.patch_embed(x)
        curr_patches = x.size(1)
        # print(x.shape)
        # exit()
        ...
        if T != self.temporal_embed.size(1):
            time_embed = self.temporal_embed.transpose(1, 2)
        ...
        if return_feat_3d:
            ...
            x = self.pre_logits(x)
            visual3d = x[:,1:]
            visual3d = rearrange(visual3d, 'b (h w t) d -> b h w t d', w=W, t=f)
            cls_token = x[:,0]
            return visual3d, cls_token
    ...
    def forward(self, x, return_feat_3d=False):
        # Note:  B C T H W => B T C H W
        # The default input order is different from the one in Frozen-in-Time
        x = x.permute(0, 2, 1, 3, 4).contiguous()
        if return_feat_3d:
            x, cls_token = self.forward_features(x, return_feat_3d=True)
            if self.image_projection is not None:
                cls_token = cls_token @ self.image_projection
            return x, cls_token
