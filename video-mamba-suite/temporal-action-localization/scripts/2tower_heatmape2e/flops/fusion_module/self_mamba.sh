#!/bin/bash
# keywords: SelfMambaEarlyFusion
config=configs/2stage/2tower/selfmamba/mamba_swallow_i3d_secondstage_2tower_ep30_acu4_loadheatmap.yaml
...
#     --output load_heatmap \
#     --tower_name SelfMambaEarlyFusion \
#     --resume resume

config=configs/2stage/2tower/selfmamba/mamba_swallow_i3d_secondstage_2tower_ep30_acu4_loadheatmap_vw0.6.yaml
vw=$(grep 'vw:' "$config" | awk -F ':' '{print $2}' | xargs)
vw=${vw:0:3}
...
nohup python eval2tower.py \
    ...
    --image_size 128 \
    --calflops \
    --tower_name SelfMambaEarlyFusion > outputs/${base_name}/eval_${vw}_flops.log