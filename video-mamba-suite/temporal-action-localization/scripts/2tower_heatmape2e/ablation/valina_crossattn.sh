#!/bin/bash
# keywords: CrossAttnEarlyFusion-PreNorm
config=configs/2stage/2tower/valina-crossattn/mamba_swallow_i3d_secondstage_2tower_ep30_acu4_loadheatmap.yaml
...
config=configs/2stage/2tower/valina-crossattn/mamba_swallow_i3d_secondstage_2tower_ep30_acu4_loadheatmap_vw0.6.yaml
vw=$(grep 'vw:' "$config" | awk -F ':' '{print $2}' | xargs)
vw=${vw:0:3}
...
nohup python eval2tower.py \
    ...
    --heatmap_size 56 \
    --image_size 128 \
    --tower_name CrossAttnEarlyFusion-PreNorm > outputs/${base_name}/eval_${vw}.log