#!/bin/bash
config=configs/2stage/2tower/crossattn/mamba_swallow_i3d_secondstage_2tower_dino_l3_ep30_acu4_loadvideomamba.yaml
...
#     --output load_videomamba \
#     --tower_name DINOAttnEarlyFusion \
#     --filter_backbone2 spatial_or_temporal_net \
#     --resume resume \
#     --enable_branch_eval
...
echo "Redirecting output to outputs/${base_name}/lastep_eval_${vw}.log"
...
nohup python eval2tower.py \
    ...
    --image_size 128 \
    --last_epoch \
    --tower_name DINOAttnEarlyFusion > outputs/${base_name}/lastep_eval_${vw}.log
