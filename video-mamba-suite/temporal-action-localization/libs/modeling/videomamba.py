...
from mamba_ssm.modules.mamba_video import Mamba
...
class VisionMamba(nn.Module):
    def __init__(
        ...
        assert feature_output_method in ['cls', 'mean', 'avg', 'all', 'none', 'pop_cls', 'no_cls'], f'Unsupported feature_output_method: {feature_output_method}'
        self.feature_output_method = feature_output_method
    ...
    def forward_features(self, x, inference_params=None):
        ...
        if self.feature_output_method == 'mean' or self.feature_output_method == 'avg':
            hidden_states = hidden_states[:, 1:, :]
            hidden_states = hidden_states.mean(dim=1)
            return hidden_states
        elif self.feature_output_method == 'cls':
            # return only cls token
            return hidden_states[:, 0, :]
        elif self.feature_output_method == 'none' or self.feature_output_method == 'all':
            return hidden_states
        elif self.feature_output_method == 'pop_cls' or self.feature_output_method == 'no_cls':
            hidden_states = hidden_states[:, 1:, :]
            return hidden_states
...
if __name__ == '__main__':
    ...
    model = VisionMamba(
        ...
        num_classes=1,
        use_checkpoint=False,
        feature_output_method='pop_cls',
    ).cuda()
    input_tensor = torch.rand(1, 1, num_frames, img_size, img_size).cuda()
